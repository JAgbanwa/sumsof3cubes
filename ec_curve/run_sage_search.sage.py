...
# This file was *autogenerated* from the file /Users/jamalmac/Desktop/sumsof3cubes/ec_curve/run_sage_search.sage
from sage.all_cmdline import *   # import sage library

_sage_const_120 = Integer(120); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_200 = Integer(200); _sage_const_66 = Integer(66); _sage_const_10 = Integer(10)
...
def load_state():
    try:
        ...
        return data.get("last_radius", _sage_const_0 )
    except Exception:
    ...
    try:
        ...
        for line in proc.stdout.splitlines():
            if line.startswith("SOLUTION"):
                parts = line.split()
                results.append((int(parts[_sage_const_1 ]), int(parts[_sage_const_2 ]), int(parts[_sage_const_3 ])))
        if proc.returncode != _sage_const_0  and proc.stderr.strip():
            print(f"  [STDERR n={n}] {proc.stderr.strip()[:_sage_const_200 ]}", flush=True)
...
if MASTER_FILE.exists():
    for line in open(MASTER_FILE):
        ...
        if len(parts) == _sage_const_3 :
            try:
                master_seen.add((int(parts[_sage_const_0 ]), int(parts[_sage_const_1 ]), int(parts[_sage_const_2 ])))
            except ValueError:
                pass

master_f = open(str(MASTER_FILE), "a", buffering=_sage_const_1 )
            ...
            if timed_out:
                ...
                for trip in sols:
                    if trip not in master_seen:
                        ...
                        master_f.flush()
                        total_new += _sage_const_1 
                        print(f"  SOLUTION  n={nn:>10}  x={xx:>22}  y={yy:>22}",
                              flush=True)
                if sols or radius % _sage_const_10  == _sage_const_0 :
